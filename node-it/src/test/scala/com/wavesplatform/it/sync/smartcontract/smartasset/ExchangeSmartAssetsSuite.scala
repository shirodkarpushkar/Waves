...
class ExchangeSmartAssetsSuite extends BaseTransactionSuite with CancelAfterFailure with NTPTime {
  ...
  test("require using a certain matcher with smart accounts") {
    ...
    miner.setAssetScript(sAsset, firstKeyPair, setAssetScriptFee, sUpdated, waitForTx = true)

    assertApiError(
      miner.signedBroadcast(exchangeTx(smartPair, smartMatcherFee + smartFee, smartMatcherFee + smartFee, ntpTime, 3, 2, acc1, acc0, acc2)),
  ...
  test("AssetPair from smart assets") {
    ...
    miner.transfer(secondKeyPair, firstAddress, 1000, minFee + smartFee, Some(assetB), waitForTx = true)
    miner.transfer(firstKeyPair, secondAddress, 1000, minFee + smartFee, Some(assetA), waitForTx = true)
    ...
    miner.setAssetScript(assetA, firstKeyPair, setAssetScriptFee, script, waitForTx = true)
    miner.setAssetScript(assetB, secondKeyPair, setAssetScriptFee, script, waitForTx = true)
    ...
    withClue("check fee for smart accounts and smart AssetPair - extx.fee == 0.015.waves") {
      ...
      assertBadRequestAndMessage(
        miner.signedBroadcast(exchangeTx(smartAssetPair, smartMatcherFee + smartFee, smartMatcherFee + smartFee, ntpTime, 2, 2, acc1, acc0, acc2)),
  ...
  test("use all functions from RIDE for asset script") {
    ...
    List(script1, script2, script3)
      .map { i =>
        ...
        miner.signedBroadcast(exchangeTx(smartPair, smartMatcherFee, smartMatcherFee, ntpTime, 2, 3, acc1, acc0, acc2), waitForTx = true)
      }
  }
}
