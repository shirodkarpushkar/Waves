...
class DataTransactionGrpcSuite extends GrpcBaseTransactionSuite {
  ...
  test("cannot put data without having enough waves") {
    for (v <- dataTxSupportedVersions) {
      ...
      assertGrpcError(
        miner.putData(firstAcc, data, firstBalance + 1, version = v),
  ...
  test("cannot broadcast data transaction with insufficient fee") {
    ...
    for (v <- dataTxSupportedVersions) {
      ...
      val firstEffBalance = miner.wavesBalance(firstAddress).effective
      assertGrpcError(
        miner.putData(firstAcc, data, minFee - 1),
  ...
  test("try to send tx above limits of key, value and size") {
    for (v <- dataTxSupportedVersions) {
      ...
      assertGrpcError(miner.putData(firstAcc, extraValueData, calcDataFee(extraValueData, v), version = v),
        "Too big sequence requested", Code.INVALID_ARGUMENT)
      ...
      assertGrpcError(miner.putData(firstAcc, largeBinData, calcDataFee(largeBinData, v), version = v),
        "Too big sequence requested", Code.INVALID_ARGUMENT)

      val largeStrData = List.tabulate(5)(n => DataEntry(tooBigKey, DataEntry.Value.StringValue("A" * maxValueSize)))
      assertGrpcError(miner.putData(firstAcc, largeStrData, calcDataFee(largeStrData, v), version = v),
        "Too big sequence requested", Code.INVALID_ARGUMENT)

      val tooManyEntriesData = List.tabulate(maxEntryCount + 1)(n => DataEntry(s"key$n", DataEntry.Value.IntValue(10)))
      assertGrpcError(miner.putData(firstAcc, tooManyEntriesData, calcDataFee(tooManyEntriesData, v), version = v),
        "Too big sequence requested", Code.INVALID_ARGUMENT)
    }
  }
  ...
  test("try to make address with 1000 DataEntries") {
    for (v <- dataTxSupportedVersions) {
      ...
      val txIds = dataSet.grouped(100).map(_.toList).map(data => PBTransactions.vanilla(miner.putData(fourthAcc, data, calcDataFee(data, v), version = v)).explicitGet().id().toString)
      ...
      miner.getDataByKey(fourthAddress, s"blob$r") shouldBe List(DataEntry(s"blob$r", DataEntry.Value.BinaryValue(ByteString.copyFrom(Array[Byte](127.toByte, 0, 1, 1)))))
